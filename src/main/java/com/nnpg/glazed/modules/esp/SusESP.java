...
public class SusESP extends Module {
    ...
    private final Setting<Boolean> detectDripstone = sgDetect.add(new BoolSetting.Builder()
        .name("dripstone").description("Detect dripstone formations 6–7 blocks long.").defaultValue(true).build());
    private final Setting<Boolean> detectVines = sgDetect.add(new BoolSetting.Builder()
        .name("vines").description("Detect vines 9–11 blocks long.").defaultValue(true).build());
    private final Setting<Boolean> detectGlowBerries = sgDetect.add(new BoolSetting.Builder()
        .name("glow-berries").description("Detect cave vines 8 blocks long.").defaultValue(true).build());
    private final Setting<Boolean> detectTrader = sgDetect.add(new BoolSetting.Builder()
        .name("wandering-trader").description("Detect Wandering Trader with 2 trader llamas.").defaultValue(true).build());
    private final Setting<Boolean> detectPillagers = sgDetect.add(new BoolSetting.Builder()
        .name("pillagers").description("Detect pillagers in the area.").defaultValue(true).build());
    private final Setting<Boolean> detectCobbledDeepslate = sgDetect.add(new BoolSetting.Builder()
        .name("cobbled-deepslate").description("Detect cobbled & rotated deepslate between Y -64 and 45.").defaultValue(true).build());
    ...
    private static final Color COLOR_DRIPSTONE  = new Color(245, 245, 245, 100);
    ...
    private static final Color COLOR_DEEPSLATE  = new Color(120, 120, 140, 100);

    public SusESP() { super(GlazedAddon.esp, "sus-esp", "Highlights suspicious formations and entities underground."); }

    @Override
    public void onActivate() { highlights.clear(); suspiciousEntities.clear(); }
    @Override
    public void onDeactivate() { highlights.clear(); suspiciousEntities.clear(); }
    ...
    private void scanChunk(WorldChunk chunk) {
        int startX = chunk.getPos().getStartX(), endX = chunk.getPos().getEndX();
        int startZ = chunk.getPos().getStartZ(), endZ = chunk.getPos().getEndZ();
        ...
        for (int x = startX; x <= endX; x++) {
            for (int z = startZ; z <= endZ; z++) {
                for (int y = Y_MIN; y <= Y_MAX; y++) {
                    ...
                    if (detectCobbledDeepslate.get() && isDeepslateTarget(block)) highlights.put(pos, COLOR_DEEPSLATE);

                    if (detectDripstone.get() && block == Blocks.POINTED_DRIPSTONE &&
                        measureVerticalLength(pos, Blocks.POINTED_DRIPSTONE) >= 6) highlights.put(pos, COLOR_DRIPSTONE);

                    if (detectVines.get() && block == Blocks.VINE &&
                        measureVerticalLength(pos, Blocks.VINE) >= 9) highlights.put(pos, COLOR_VINES);

                    if (detectGlowBerries.get() && (block == Blocks.CAVE_VINES || block == Blocks.CAVE_VINES_PLANT) &&
                        measureVerticalLength(pos, Blocks.CAVE_VINES, Blocks.CAVE_VINES_PLANT) == 8) highlights.put(pos, COLOR_GLOWBERRY);
                }
            }
        }
    }
    ...
    private int measureVerticalLength(BlockPos start, Block... matchBlocks) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            boolean match = false;
            for (Block m : matchBlocks) if (block == m) { length++; match = true; break; }
            if (!match) break;
        }
        return length;
    }
    ...
    @EventHandler
    private void onEntityRemoved(EntityRemovedEvent event) { suspiciousEntities.remove(event.entity); }
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        Vec3d tracerStart = eye.add(look.normalize().multiply(0.5));

        // connected block groups for single tracer
        ...
        for (BlockPos pos : highlights.keySet()) {
            ...
            Box groupBox = new Box(minX, minY, minZ, maxX+1, maxY+1, maxZ+1);
            event.renderer.box(groupBox, color, color, ShapeMode.Both, 1);
        ...
        for (Entity e : suspiciousEntities) {
            ...
            Vec3d center = new Vec3d(e.getX(), e.getY() + e.getHeight()/2.0, e.getZ());
            event.renderer.line(tracerStart.x, tracerStart.y, tracerStart.z, center.x, center.y, center.z, color);

            if (e instanceof WanderingTraderEntity trader) {
                for (TraderLlamaEntity llama : trader.getWorld().getEntitiesByClass(TraderLlamaEntity.class, trader.getBoundingBox().expand(32), l -> true)) {
    ...
    private void floodFill(BlockPos start, Color color, List<BlockPos> group, Set<BlockPos> visited) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (BlockPos offset : Arrays.asList(pos.add(1,0,0), pos.add(-1,0,0), pos.add(0,1,0), pos.add(0,-1,0), pos.add(0,0,1), pos.add(0,0,-1))) {
                queue.add(offset);
            }
        }
    }
